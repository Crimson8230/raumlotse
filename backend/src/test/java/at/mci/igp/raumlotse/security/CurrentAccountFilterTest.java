...
class CurrentAccountFilterTest {
        ...
        void missingPersistedAccountClearsContextBeforeContinuingToSecurityChecks(
                ...
                assertThat(output.getAll()).contains(
                                "authentication_failure code=AUTH_REQUIRED reason=account_missing status=401");
        }
        ...
        void accountStoreFailureReturnsSafe503WithoutCallingBusinessChain(
                ...
                new CurrentAccountFilter(repository, new JsonMapper()).doFilter(new MockHttpServletRequest(), response,
                                chain);
                ...
                assertThat(output.getAll()).contains(
                                "authentication_failure code=AUTH_UNAVAILABLE reason=account_lookup status=503")
                                .doesNotContain("private database detail");
        }
}
