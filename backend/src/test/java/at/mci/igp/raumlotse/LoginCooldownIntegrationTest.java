...
class LoginCooldownIntegrationTest extends AbstractIntegrationTest {
        ...
        JdbcTemplate jdbc;
        @Autowired
        at.mci.igp.raumlotse.service.LoginAttemptIdentity identities;
        @Autowired
        ...
        void registeredAndUnknownAddressesReceiveTheSameFifthFailureAndFixedCooldown() throws Exception {
                ...
                accounts.saveAndFlush(new UserAccount(registered, "Cooldown User",
                ...
                String knownDetail = com.jayway.jsonpath.JsonPath.read(knownFifth.getResponse().getContentAsString(),
                                "$.detail");
                String unknownDetail = com.jayway.jsonpath.JsonPath
                                .read(unknownFifth.getResponse().getContentAsString(), "$.detail");
                String knownCode = com.jayway.jsonpath.JsonPath.read(knownFifth.getResponse().getContentAsString(),
                                "$.code");
                Integer retryAfter = com.jayway.jsonpath.JsonPath.read(knownFifth.getResponse().getContentAsString(),
                                "$.retryAfterSeconds");
                ...
                java.sql.Timestamp deadline = jdbc.queryForObject(
                                "select blocked_until from login_attempt_state where identity_key = ?",
                ...
                var restartedService = new LoginAttemptService(authentication, states, identities, canonicalizer, jdbc,
                                transactionManager);
        ...
        private MockHttpSession loginCorrectly(String email, String password) throws Exception {
                ...
                String token = com.jayway.jsonpath.JsonPath.read(bootstrap.getResponse().getContentAsString(),
                                "$.token");
        ...
        private SessionToken bootstrap() throws Exception {
                ...
                return new SessionToken((MockHttpSession) result.getRequest().getSession(false),
                                com.jayway.jsonpath.JsonPath.read(result.getResponse().getContentAsString(),
                                                "$.token"));
        }
        ...
        private String tokenFrom(MockHttpSession session) throws Exception {
                MvcResult fresh = mvc.perform(get("/api/auth/csrf").session(session)).andExpect(status().isOk())
                                .andReturn();
        ...
        private org.springframework.test.web.servlet.ResultActions postCredentials(String email, String password,
                ...
                return mvc.perform(post("/api/auth/login").session((MockHttpSession) session)
