...
class AuthControllerTest {
        ...
        void loginRequiresCsrfAndReturnsOnlySafeIdentity() throws Exception {
                ...
                when(attempts.authenticate(eq("user@example.test"), eq(" exact "), any()))
                                .thenAnswer(invocation -> {
                                        invocation.<java.util.function.Consumer<UserAccount>>getArgument(2)
                                                        .accept(account);
                                        return new LoginAttemptService.Attempt(LoginAttemptService.Outcome.SUCCESS,
                                                        account, 0);
                                });
                ...
                var authenticatedSession = (org.springframework.mock.web.MockHttpSession) login.getRequest()
                                .getSession(false);
        ...
        void wrongCredentialsHaveGenericSafeResponse() throws Exception {
                ...
                when(attempts.authenticate(eq("nobody@example.test"), eq("wrong"), any()))
                                .thenReturn(new LoginAttemptService.Attempt(
                                                LoginAttemptService.Outcome.INVALID_CREDENTIALS, null, 0));

                mvc.perform(post("/api/auth/login")
                                .session((org.springframework.mock.web.MockHttpSession) bootstrap.getRequest()
                                                .getSession(false))
                                .header("X-CSRF-TOKEN", token).contentType("application/json")
        ...
        void fifthFailureAndActiveCooldownReturnRetryMetadata() throws Exception {
                ...
                when(attempts.authenticate(eq("fifth@example.test"), eq("wrong"), any()))
                                .thenReturn(new LoginAttemptService.Attempt(
                                                LoginAttemptService.Outcome.INVALID_CREDENTIALS, null, 900));
                mvc.perform(post("/api/auth/login")
                                .session((org.springframework.mock.web.MockHttpSession) bootstrap.getRequest()
                                                .getSession(false))
                                .header("X-CSRF-TOKEN", token).contentType("application/json")
                ...
                when(attempts.authenticate(eq("fifth@example.test"), eq("right"), any()))
                                .thenReturn(new LoginAttemptService.Attempt(LoginAttemptService.Outcome.COOLDOWN, null,
                                                899));
                mvc.perform(post("/api/auth/login")
                                .session((org.springframework.mock.web.MockHttpSession) next.getRequest()
                                                .getSession(false))
                ...
                mvc.perform(post("/api/auth/login")
                                ...
                                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.content()
                                                .string(org.hamcrest.Matchers
                                                                .not(org.hamcrest.Matchers.containsString("secret"))));
        }
        ...
        void authenticationStoreFailureIsSanitizedAs503() throws Exception {
                ...
                mvc.perform(post("/api/auth/login")
                                ...
                                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.content()
                                                .string(org.hamcrest.Matchers.not(org.hamcrest.Matchers
                                                                .containsString("private database sentinel"))));
        }
}
