...
1. **Given** an active room with a current reservation, **When** the screen view is opened, **Then** it displays the current date and time, room name, reservation title, lecturer name, reservation start time, and reservation end time.
...
3. **Given** a reservation with a title or lecturer name containing spaces or special characters, **When** the screen view renders, **Then** the complete value is shown without being silently changed.
...
2. **Given** a long room name, reservation title, or lecturer name, **When** the screen view renders, **Then** the value remains readable through wrapping or another visible layout treatment and does not overlap another required field.
...
- If a reservation has a missing title or lecturer name, the screen MUST show a visible fallback such as "Not specified" for that field; it MUST NOT show an empty unlabeled area.
...
- **FR-004**: When a current reservation exists for the displayed room, the screen MUST display the reservation title.
- **FR-005**: When a current reservation exists for the displayed room, the screen MUST display the lecturer's name.
...
- **FR-008**: The screen MUST use reservation data from the room-reservations domain for the displayed reservation title, lecturer, start time, and end time.
...
- **FR-011**: The screen MUST provide visible fallback text for missing optional reservation title or lecturer values.
- **FR-012**: The screen MUST keep all required fields readable at the intended mockup display size, including when room names, reservation titles, or lecturer names are long.
...
- **Reservation**: The room-reservations record that supplies the current event title, lecturer name, start time, and end time.
...
- **SC-001**: In 100% of test cases with valid linked room and reservation data, the screen displays the room name, reservation title, lecturer name, start time, and end time from the matching records.
...
- The reservation data model already contains, or will expose for this feature, a reservation title and lecturer name in addition to its start and end times.
