...
- Q: Can users update additional equipment requests when modifying a RESERVED booking, or are edits strictly limited to notes and expected attendee count? (CHK012) → A: Strictly notes and attendee count only: Modifications to existing `RESERVED` bookings are strictly limited to notes and expected attendee count (within seating capacity). Modifying additional equipment, seating layout, room, or time window is prohibited and requires cancelling and re-booking.
...
- **Attempting to Reschedule or Modify Equipment via Edit**: What happens if a user attempts to change the room, time window, seating arrangement, or additional equipment of an existing reservation? The system MUST reject the update; schedule, layout, and equipment modifications require cancelling and creating a new reservation.
...
- **FR-018**: System MUST prevent updating the room, time window (`startTime`, `endTime`), seating arrangement, or requested additional equipment of an existing reservation; any schedule, layout, or equipment modification requires cancelling and creating a new reservation.
